...
//******************************************************************************
	...
	//Base year is hard coded to 2009
    var scwBaseYear        = 1900;

    // How many years do want to be valid and to show in the drop-down list?

	//This will show 20 more years from current year and as base year is
	//set to 2009 so year drop down will have year starting from 2009 to current year + 20
    ...
    var scwDateDisplayFormat = 'MMM D YYYY';     // e.g. 'MMM-DD-YYYY' for the US
    ...
    document.writeln(
        '<style type="text/css">'                                       +
            ...
            '/* Calendar background */'                                 +
            'table.scw                  {background-color:  #DAE0EB;}'  +
            '/* Drag Handle */'                                         +
            ...
            '/* Enabled Days */'                                        +
            '/* Week Day */'                                            +
            'td.scwCells                {background-color:  #EAEDF4;'   +
                                        'color:             #000000;}'  +
            '/* Day matching the input date */'                         +
            'td.scwInputDate            {background-color:  #CC9999;'   +
                                        'color:             #FF0000;}'  +
            '/* Weekend Day */'                                         +
            'td.scwCellsWeekend         {background-color:  #EAEDF4;'   +
                                        'color:             #000000;}'  +
            '/* Day outside the current month */'                       +
            'td.scwCellsExMonth         {background-color:  #CCCCCC;'   +
                                        'color:             #666666;}'  +
            '/* Today selector */'                                      +
            'td.scwNow                  {background-color:  #002854;'   +
                                        'color:             #FFFFFF;'   +
                                        'font-size:         8pt;'		+
                                        'font-weight:		bold;}'		+
            '/* Clear Button */'                                        +
            ...
            {function scwInputFormat()
                {var scwArrSeed = new Array(),
                 ...
                 if (scwExpValYear.exec(scwArrSeed[0])  == null ||
                    ...
                    {
                    //"scwDateValue!='mm/dd/yyyy'" this condition is added for showing date format on transaction search
                    if (scwShowInvalidDateMsg && scwDateValue!='mm/dd/yyyy' && scwDateValue!='MM/DD/YYYY' && scwDateValue!='Due Date') {                      	
                    	alert(scwInvalidDateMsg  +
                               scwInvalidAlert[0] + scwDateValue +
                               scwInvalidAlert[1]);}
                     scwBlnFullInputDate = false;
                     
                     //Below code is updated to show currecnt date in case of invalid entry
                     scwArrSeed[0] = scwDateNow.getFullYear().toString();   // Year                                     
                     scwArrSeed[1] = (scwDateNow.getMonth()+ 1).toString(); // Month
                     scwArrSeed[2] = scwDateNow.getDate().toString();       // Day
