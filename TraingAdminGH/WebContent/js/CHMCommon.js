 ...
 *--------------------------------------------------------------------------------
 *  1.1	   29/09/2005	aliasgarj		IT Defect Fix: 
 *										1. The adjustment screen does not allow negative values to be entered.
 *										Removed isDecimal() and isDecimalWithPrecison() fns
 *	1.2    08/12/2005	aliasgarj		Changed function changeStatusSelect() and changeStatus for Designation master
 *										Made the function generic to pass additional optional parameter status and pkvalue
 *  1.3    28/Mar/2006   Ashaya K       fixed defect number 101086 
 *	1.4	   18/May/2006   Tanmay			Pru Base Code merging
 *										Label used:-cms_pbs_vtn(1.3.0)_cor_cod-merge	
  *  1.5     13/06/2006   Vinay          cms_cor_mov-auto-wdraw_defect : Checking For withdraw and bussiness dates.
  *  1.6   31/08/2009    Kamlesh P      |Defect-3884|IT|error occured during decimal validation  
 * --------------------------------------------------------------------------------
 *
 */
...
/*	var strMonth=dtStr.substring(0,pos1)
	...
	year=parseInt(strYr);
	if (pos1==-1 || pos2==-1){
		alert(getErrorMsg(49,'','/'));
		return false;
	}
	if (strMonth.length<1 || month<1 || month>12){
		alert(getErrorMsg(50,'month'));
		return false;
	}
	if (strDay.length<1 || day<1 || day>31 || (month==2 && day>daysInFebruary(year)) || day > daysInMonth[month]){
		alert(getErrorMsg(50,'day'));
		return false;
	}
	if (strYear.length != 4 || year==0 || year<minYear || year>maxYear){
		alert(getErrorMsg(51,minYear,maxYear));
		return false;
	}
	if (dtStr.indexOf(dtCh,pos2+1)!=-1 || isInteger(stripCharsInBag(dtStr, dtCh))==false){
		alert(getErrorMsg(50,'date') + '. ' + getErrorMsg(49,'','/'));
		return false;
	}
return true;
}
...
/* cms_cor_mov-auto-wdraw_defect : Start*/
function compareDtEqual(firstdt,seconddt)
{
//compares if firstdate is equal to seconddate
	var day1=firstdt.substring(0,2)
	var month1=firstdt.substring(3,5)
	var year1=firstdt.substring(6);
	var day2=seconddt.substring(0,2)
...
/* cms_cor_mov-auto-wdraw_defect : End */
/**
* @author Ashwini Nadkarni
...
function changeStatus(form_name,chBox,src,status,pkvalue)
{
/*
...
/*added by ashaya k start to fix defect number 101086*/
	if(!status)
	 {
		  status = 'statusFlag';
	 } 
/*added by ashaya k end to fix defect number 101086*/
...
else
{
	...
	if ((statusFlag=="D" && !(eval('document.'+form_name+'.'+chBox).checked)))
	{
	 ...
	 function changeStatusSelect(form_name,chBox,src,status,pkvalue)
        {
  		 if(!status){
		  status = 'statusFlag';
		 }
		  ...
		  if(objArray.length)
		  {
			changeStatus(form_name,chBox,src,status,pkvalue)
		  }
		  else{
			    statusFlag=eval('document.'+form_name+'.'+status).value
				if ((statusFlag=="X" || statusFlag=="U")&& eval('document.'+form_name+'.'+chBox).checked)
...
function hasSpace(obj,fieldname)
{
  ...
  */
  ...
  /**
  * Purpose: This function can return the index of the selected value of the drop-down box.
  * Parameters:	selectList - Drop-down box object
  *				strVal - Value of the drop-down box, which should be selected
  * Returns: 	Index of the value ssuppose to be selected. Return -1 if the value not found.
  */
  function getComboxIndexByVal(selectList, strVal) {
	var boxLength = selectList.length;
	for (i = 0; i < boxLength; i++) {
		if (selectList.options[i].value == strVal) {
			return i;
		}
	}
	return -1;
...
/* start cms_pbs_vtn(1.3.0)_cor_cod-merge */

function isDecimalWithPrecision(theInputField, fieldname, allowedTotalDigits, allowedDigitsAfterDecimal, isNegativeAllowed, bDoNotShowAlert,divname)
{

  if(isDecimal(theInputField, fieldname,divname))
  {
...
//|Defect-3884|IT|error occured during decimal validation
function isDecimal(theInputField,fieldname,divname)
{
 ...
 //check if number field contains more than one decimal points '.'
  ...
  {
	  if(fieldname)
		alert(fieldname+' field cannot contain two decimal points');
	  else
		alert('The field cannot contain two decimal points');
  ...
  {
	  if(fieldname)
		alert(fieldname+' field cannot contain just a decimal');
	  else
		alert('The field cannot contain just a decimal');
  ...
  for (var i = 0 ; i < theLength ; i++)
  {
    ...
    if (theInput.charAt(i) < '0' || theInput.charAt(i) > '9')
    {
      ...
      {
		  if(fieldname)
			alert(fieldname+' field cannot contain alphabets or spaces');
		  else
			alert('This field cannot contain alphabets or spaces');
...
//|Defect-3884|IT|error occured during decimal validation
/* end cms_pbs_vtn(1.3.0)_cor_cod-merge */